...
PHASE1=../../circuits/pot12_final.ptau
BUILD_DIR=../../build/initialStage
CIRCUIT_NAME=InitialStage
...
start=`date +%s`
node "$BUILD_DIR"/"$CIRCUIT_NAME"_js/generate_witness.js "$BUILD_DIR"/"$CIRCUIT_NAME"_js/"$CIRCUIT_NAME".wasm input_initial.json "$BUILD_DIR"/witness.wtns
end=`date +%s`
echo "DONE ($((end-start))s)"
...
echo "****EXPORTING VKEY****"
start=`date +%s`
npx snarkjs zkey export verificationkey "$BUILD_DIR"/"$CIRCUIT_NAME"_1.zkey "$BUILD_DIR"/vkey.json
end=`date +%s`
echo "DONE ($((end-start))s)"

echo "****GENERATING PROOF FOR SAMPLE INPUT****"
start=`date +%s`
npx snarkjs groth16 prove "$BUILD_DIR"/"$CIRCUIT_NAME"_1.zkey "$BUILD_DIR"/witness.wtns "$BUILD_DIR"/proof.json "$BUILD_DIR"/public.json
end=`date +%s`
echo "DONE ($((end-start))s)"
...
start=`date +%s`
npx snarkjs zkey export solidityverifier "$BUILD_DIR"/"$CIRCUIT_NAME"_1.zkey verifier.sol
end=`date +%s`
echo "DONE ($((end-start))s)"

echo "****GENERATING CONTRACT CALLS****"
start=`date +%s`
cd "$BUILD_DIR"
npx snarkjs generatecall > call.txt
end=`date +%s`
echo "DONE ($((end-start))s)"

end0=`date +%s`
echo "TOTAL DONE ($((end0-start0))s)"
